...
For the security principles that apply to all integrations, see the [Quick Start](/getting-started/quickstart#security-principles).
...
# Public identifier used in ?key=pk_... (safe to expose in URLs).
# Still recommended to keep in server env for centralized config management.
OPTSTUFF_PUBLIC_KEY="pk_abc123..."

# Server-side only — secret! Never expose in browser bundles
OPTSTUFF_SECRET_KEY="sk_your_secret_key_here"
```

## Create a Signing Utility

Create `lib/optstuff-core.ts` (server-side only):
 ...
 *
 * @example
 * generateOptStuffUrl("https://cdn.example.com/photo.jpg", { width: 800 }, 3600) // expires in 1 hour
 */
...
<Callout type="warn">
Do **not** pass `generateOptStuffUrl` (or any function that uses your secret key) directly as
...
1. `examples/nextjs/src/lib/optstuff-core.ts`
...
If you also want server-generated blur placeholders from the demo, additionally copy:

- `examples/nextjs/src/lib/optstuff-blur.ts`
- `examples/nextjs/src/lib/hero-blur-config.ts`

This keeps the boundary clear:

- **Core runtime integration**: signing + loader + API route + image wrapper
- **Optional enhancement**: server blur generation
- **Demo-only UI**: hero/playground/comparison/debug components
...
- Pre-sign the hero URL in a Server Component and pass `preSigned` so the request skips `/api/optstuff` redirect.
...
2. Enable the button (`request: fresh`) and verify the URL includes `?hero-refresh=1`.
3. Check the blur debug panel for `Request Mode: fresh` and `Network Request: yes`.
4. Disable it again (or remove `?hero-refresh=1`) to return to normal cache behavior.

`HERO_FORCE_REFRESH_URL_TTL_SECONDS` (in `examples/nextjs/src/app/page.tsx`) controls force-refresh signed URL validity (`exp`).

- Keep it short for local debug so force refresh can rotate URLs quickly.
- Recommended range: `10-30` seconds.
- Avoid very low values (for example `1`), which can cause intermittent `Image unavailable` during rapid reloads due to expiry race.
...
| `403` Source domain not allowed | Image origin not in project's allowed source domains | Add the domain in Dashboard → Project → Settings → Domain Security → Image Sources |
| `403` Invalid referer | Your website not in project's allowed referer domains | Add the domain in Dashboard → Project → Settings → Domain Security → Authorized Websites |
