---
title: Next.js
description: Step-by-step guide to integrating OptStuff image optimization into a Next.js application, including a signing utility, Server Component, API Route, and next/image loader examples.
---
...
### Option 3: `next/image` Loader

You can use OptStuff as a custom loader for `next/image`, which lets Next.js handle responsive `srcSet` generation automatically.

Create `lib/optstuff-loader.ts`:
...
Use it with the `next/image` component:
...
- [CDN and Caching](/guides/cdn-caching) — CDN integration and `next/image` loader
