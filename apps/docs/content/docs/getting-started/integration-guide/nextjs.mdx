...
Using a bucketed expiration keeps signatures stable within each bucket window, which improves CDN cache hit rates on Vercel and helps preserve approximate URL expiry, but may extend effective validity by up to one bucket due to rounding.
