...
Using a bucketed expiration keeps signatures stable within each bucket window, which improves CDN cache hit rates on Vercel while preserving URL expiry.
