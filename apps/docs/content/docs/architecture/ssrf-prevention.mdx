...
description: How our image proxy architecture prevents Server-Side Request Forgery (SSRF) — the design flaw, the attack vector, and the fix.
---
...
```mermaid title="Image API layers"
flowchart TD
  A["API Gateway Layer<br/>Authentication · Rate Limiting · Input Validation"]
  B["Validation Layer<br/>Referer check · Source domain allowlist check"]
  C["Processing Layer<br/>IPX image engine (fetch + transform)"]
  D["Internet"]
...
```mermaid title="SSRF via open redirect"
flowchart TD
...
```mermaid title="Redirects blocked at fetch"
flowchart TD
