...
|--------|-------|---------|-------------|
| Aggregated usage | `usage_record` | Aggregate totals (`totalRequests`, `totalBytes`) | Best-effort aggregate (hot-path Redis increments can be dropped on Redis failures; eventually consistent by flush cadence) |
