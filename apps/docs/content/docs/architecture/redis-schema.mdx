---
title: Redis Schema
description: Architecture documentation — Redis key schema, cache-aside pattern, sliding window rate limiting, write throttling, and data durability.
---
...
| `usage:buffer:minute:` | Buffered usage counters (minute bucket hash) | 3 days | `usage:buffer:minute:202603071230` |
...
## Usage Activity Throttling (SET NX)
...
Avoid per-request database upserts on the image-serving hot path while preserving authoritative usage totals in `usage_record`.
...
5. Upserts rows into `usage_record` with additive updates.
6. Deletes flushed Redis bucket.
...
|----------|-------------|
| [Architecture Overview](/architecture/overview) | Recommended architecture reading path |
| [Request Lifecycle](/architecture/request-lifecycle) | Runtime request path that consumes these Redis keys |
