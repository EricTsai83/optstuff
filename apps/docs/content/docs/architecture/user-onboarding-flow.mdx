...
description: Architecture documentation — complete user onboarding journey from sign-up to first API key, including team creation, project setup, and documentation links strategy.
---
...
```mermaid title="User onboarding path"
flowchart TD
...
- **Image Sources** — where original images are hosted (e.g., `https://images.unsplash.com`, `https://cdn.mysite.com`). Subdomains are included automatically.
- **Authorized Websites** (optional) — which websites can display optimized images. Leave empty to allow all. These can be configured later in Settings.
...
3. **Combined env block** — Variables shown in `.env` format (`OPTSTUFF_BASE_URL`, `OPTSTUFF_PROJECT_SLUG`, `OPTSTUFF_SECRET_KEY`, `OPTSTUFF_PUBLIC_KEY`) with one copy button. The preview masks the secret, but copy payload includes plaintext.
...
1. Configure image sources (link to Settings)
2. API key ready
...
- [Create API Key Flow](/architecture/create-api-key-flow) — Detailed API key creation and encryption flow
- [Control Plane and Multi-Tenancy](/architecture/control-plane-and-tenancy) — Team/project ownership and access model
...
- [Next.js Integration](/getting-started/integrate-nextjs) — How to integrate OptStuff
