...
description: Design and rationale for the scheduled maintenance pipeline, including retry processing, request-log cleanup, and expired API key revocation.
---
...
4. **Low operational overhead**: works on Vercel Free/Hobby without extra queue infrastructure.
...
flowchart TD
  ...
  D --> E["Then run in parallel:<br/>1) cleanupOldRequestLogs<br/>2) revokeExpiredApiKeys"]
...
- Uses one cron route and Postgres only (no extra infrastructure).
