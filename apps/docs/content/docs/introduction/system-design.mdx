...
- **Observability note:** when **Redis unavailable** and the **rate limiter fails open**, create two correlated alerts: one for Redis connectivity errors/instance DOWN (Redis errors, connection failures, or sentinel alerts), and one for abnormal `request_rate` or elevated `rate_limiter_fallback_count`; runbook action: investigate Redis first, then apply temporary stricter upstream limits or WAF rules until Redis recovers.
...
- [How OptStuff Is Built (Architecture)](/architecture/how-optstuff-is-built)
- [System Overview (Architecture)](/architecture/system-overview)
