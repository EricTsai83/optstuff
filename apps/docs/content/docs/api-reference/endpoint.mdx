...
For `GET` transform/fetch failures, the endpoint uses `502` as the primary failure status. Upstream `404` and `410` are passed through unchanged.
...
| 9 | Route by method (`GET` transform / `HEAD` fast path) | `502` (GET transform/fetch failures; upstream `404`/`410` passthrough) |
...
- `HEAD` requests run the full auth/abuse checks but skip image transformation, and avoid loading transform dependencies.
