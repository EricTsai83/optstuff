...
OptStuff exposes a single REST endpoint for image optimization. All transformations, authentication, and caching are encoded in the URL. For a step-by-step integration tutorial, see the [Next.js Integration](/getting-started/integrate-nextjs) guide.
...
| `X-Original-Size-Sampled` | `1` when an upstream `HEAD` probe is actually scheduled for this request, otherwise `0` |
...
| Status | Cause |
|--------|-------|
| 400 | Invalid path format, unsupported operation, or invalid operation value |
...
| 500 | Internal processing failure, or upstream fetch/probe failure without a mappable status code (rare) |
...
| 3 | Parse operations/image path and validate operation constraints | `400` |
...
| 9 | Route by method (`GET` transform / `HEAD` fast path probe) | `502` (primary upstream fetch/probe/transform failure), `404`/`410` passthrough, rare `500` fallback |
...
- [Next.js Integration](/getting-started/integrate-nextjs) — Step-by-step integration
