...
| 401 | API key has been revoked | Key was explicitly revoked | Create a new API key from the [dashboard](/getting-started/dashboard-setup) |
...
| 403 | Forbidden: Source domain not allowed | Image source domain not in key's allowed list | Add the domain in key settings (see [Domain Whitelisting](/guides/domain-whitelisting)) |
...
| 500 | Image processing failed | Internal error during optimization | Check that the source image is accessible and in a [supported format](/api-reference/image-operations#supported-formats) |
