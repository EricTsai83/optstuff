...
| 401 | API key has been revoked | Key was explicitly revoked | Create a new API key from the [dashboard](/docs/getting-started/dashboard-setup) |
| 401 | API key has expired | Key is past its `expiresAt` date | Create a new key or [rotate](/docs/guides/key-management#rotation) the existing one |
...
| 403 | Forbidden: Invalid referer | Referer header doesn't match project's allowed domains | Add your domain in project settings (see [Domain Whitelisting](/docs/guides/domain-whitelisting)) |
| 403 | Forbidden: Source domain not allowed | Image source domain not in key's allowed list | Add the domain in key settings (see [Domain Whitelisting](/docs/guides/domain-whitelisting)) |
| 404 | Project not found | `projectSlug` doesn't match any project | Check the project slug is correct |
| 429 | Rate limit exceeded | Per-minute or per-day limit reached | Wait for `Retry-After` seconds, or [increase limits](/docs/guides/rate-limiting) |
| 500 | Image processing failed | Internal error during optimization | Check that the source image is accessible and in a [supported format](/docs/api-reference/image-operations#supported-formats) |
...
| `details` | `400` (invalid image URL), `500` | Additional context about the failure |
...
- [URL Signing](/docs/guides/url-signing) — Signature formula and code examples
- [Domain Whitelisting](/docs/guides/domain-whitelisting) — Configuring allowed domains
- [Rate Limiting](/docs/guides/rate-limiting) — Rate limit configuration and tuning
