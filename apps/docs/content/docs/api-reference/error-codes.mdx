...
`HEAD` requests use a fast path (no image transformation), so transformation-related failures (`500` / `502`) apply to `GET` requests.
...
| 502 | Image processing failed | Upstream fetch/transform failure while processing a `GET` request | Verify source image availability and retry; check upstream origin stability |
