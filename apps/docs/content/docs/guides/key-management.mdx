...
This guide covers the complete lifecycle of OptStuff API keys and recommended practices for managing them securely.
...
|-----------|--------|---------|------------|
| Public Key | `pk_` + 25 characters | Identifies requests in URLs | Viewable anytime in dashboard |
| Secret Key | `sk_` + base64url | Signs URLs (HMAC-SHA256) | **Shown only once** at creation/rotation |

For details on how keys are used in requests, see [URL Signing](/docs/guides/url-signing).

## Key Lifecycle

### Creation

1. Create a key from the dashboard (see [Dashboard Setup](/docs/getting-started/dashboard-setup))
...
Rotation **revokes the old key** and **creates a new one** atomically in a single database transaction. The new key inherits all settings from the old key.
...
Set an optional expiration date when creating a key. After this date, the key is automatically rejected. This limits the damage window if a key is compromised and not noticed.
...
- [Dashboard Setup](/docs/getting-started/dashboard-setup) — How to create and manage keys in the UI
- [URL Signing](/docs/guides/url-signing) — How keys are used to sign requests
