...
```mermaid title="Custom next/image loader flow"
sequenceDiagram
  participant Browser
  participant Route as Next.js API Route (/api/optstuff)
  participant OptStuff

  Note over Browser: next/image renders srcSet and browser picks best width (viewport + DPR)
  Browser->>Route: GET /api/optstuff?url=...&w=1080&q=80&f=webp&fit=cover
  Route->>Route: Sign URL server-side (sk_... stays on server)
  Route-->>Browser: 302 Redirect -> signed OptStuff URL
  Browser->>OptStuff: GET signed image URL
  OptStuff-->>Browser: Optimized image + 1yr cache headers
```
