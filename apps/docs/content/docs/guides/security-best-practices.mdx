...
See [URL Signing](/docs/guides/url-signing) for implementation details.

### Signature Expiration

The optional `exp` parameter limits how long a signed URL is valid. Recommended expiration times depend on the use case — see [URL Signing](/docs/guides/url-signing#signature-expiration).

### Domain Whitelisting

Two-layer domain control restricts both who can use the service (referer domains) and which images can be processed (source domains). See [Domain Whitelisting](/docs/guides/domain-whitelisting) for configuration.

### Rate Limiting

Per-key rate limits prevent abuse and contain the impact of compromised keys. See [Rate Limiting](/docs/guides/rate-limiting) for configuration.
...
- [URL Signing](/docs/guides/url-signing) — Signature generation and code examples
- [Domain Whitelisting](/docs/guides/domain-whitelisting) — Domain configuration
- [Rate Limiting](/docs/guides/rate-limiting) — Rate limit configuration
- [Key Management](/docs/guides/key-management) — API key lifecycle best practices
