...
const expiresAt = Math.ceil(rawExp / bucketSeconds) * bucketSeconds;
...
const url = createSignedUrl({
  ...
  expiresAt, // bucketed expiration for better cache reuse
});
```
...
- Round the expiration to a bucket boundary (for example the next hour)
