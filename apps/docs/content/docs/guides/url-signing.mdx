...
For a higher-level overview, see [Core Concepts](/introduction/core-concepts). For background on what HMAC is (versus hashing or encryption), see [What Is HMAC?](/guides/hmac-introduction). For a complete integration walkthrough, see the [Next.js Integration](/getting-started/integrate-nextjs) guide.
...
```mermaid title="URL signing actors"
sequenceDiagram
  ...
  YourServer->>OptStuff: GET https://images.example.com/api/v1/.../path?key=pk&sig=...
...
- [Next.js Integration](/getting-started/integrate-nextjs) — Full integration walkthrough
