...
> **Before modifying:** back up the `api_keys` table and validate that your values fall within the ranges above (the schema enforces integer type but not range — out-of-range values may cause unexpected behaviour). If you are unsure about the correct limits for your use case, contact support.
...
- The per-day limit is checked **before** the per-minute limit to minimize token waste (see [Redis Schema](/internal/redis-schema) for details).
