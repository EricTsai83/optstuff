...
> **Note:** `allowedRefererDomains` is a best-effort browser-side signal enforced by `validateReferer()` for hotlinking protection, not a strong authentication guarantee. When `allowedRefererDomains` is non-empty, `validateReferer()` blocks requests only when a `Referer` is present and not allowed; requests **without** a `Referer` (for example, server-to-server calls or privacy-stripping browser policies) are intentionally allowed. For strict caller validation, use short-lived signed URLs. See [Referer Security Model](/guides/referer-security-model) for details.
