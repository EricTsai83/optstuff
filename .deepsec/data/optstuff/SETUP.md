# Agent setup for `optstuff`

This is a deepsec scanning workspace. Project `optstuff` was just registered
(target: `..`). Setup is incomplete — `data/optstuff/INFO.md`
...
2. **Fill in `data/optstuff/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id optstuff
pnpm deepsec process --project-id optstuff
```
